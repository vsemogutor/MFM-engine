package userAction
{
	...
	public class UserActionRemoveCliff extends UserActionBase
	{
		private var _mouseDown:Boolean;
		private var _brushSize:int;
		
		public function UserActionRemoveCliff(brushSize:int)
		{
			...
			if (cTile)
			{
				for (var i:int = cTile.xindex - _brushSize + 1; i < cTile.xindex + _brushSize; ++i)
				{
					for (var j:int = cTile.yindex - _brushSize + 1; j < cTile.yindex + _brushSize; ++j)
					{
						...
						if (tile)
						{
							TilesetMapEditor.instance().removeCliff(tile);
						}
					}				
				}
			}
